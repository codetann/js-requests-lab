...
sayHelloButton.addEventListener("mouseout", updateColor);
...
const repeatMyParam = () => {
  //YOUR CODE HERE
  axios.get("http://localhost:3000/repeat/Tanner").then((res) => {
...
const queryTest = () => {
  //YOUR CODE HERE
  axios
    .get("http://localhost:3000/query-test?name=Tanner&age=25")
    .then((res) => {
      // ... code to go here
...
/*
    ...
    1: Send no queries on the URL -- what happened? 
        - "you sent an empty query!" and error in node server

    2: Send more than 1 query on the URL -- what happened? 
        - Received an object pack with the message, you sent more than one query
            and a property queries with all of the querys
*/
